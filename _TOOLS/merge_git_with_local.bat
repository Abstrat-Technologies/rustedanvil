...
:diffcheck 
IF EXIST "G:\repos\rustedanvil\_TOOLS\diff.txt" (
	rm "G:\repos\rustedanvil\_TOOLS\diff.txt"
	GOTO :diff
)
...
diff -q %remote% %local% > "G:\repos\rustedanvil\_TOOLS\diff.txt"
:: Get the goats, we're summoning satan
sed --i "s/.*Files[[:space:]]//g";"s/.and[[:space:]]*.*//g";"s/.*[[:space:]]//g";"s/.*rustedanvil//g";"s/.*1.6.4//g";"s/.*\///" "G:\repos\rustedanvil\_TOOLS\diff.txt"
:: strip useless entries
grep -Fvxf blacklist.txt diff.txt > diff_temp.txt
:: Move the temp into the main file
cp "G:\repos\rustedanvil\_TOOLS\diff_temp.txt" "G:\repos\rustedanvil\_TOOLS\diff.txt"
:: temp file is useless, drag it behind the shed
rm "G:\repos\rustedanvil\_TOOLS\diff_temp.txt"
:: Ah, it's the agent of hell
sed -i -e "s_.*_G:/repos/rustedanvil/&_" "G:\repos\rustedanvil\_TOOLS\diff.txt"
:: Nuke satan
DEL *.
